 ...
 *
 * Copyright (c) 2007 JÎõÎñÎåÎÝrn Zaefferer (http://bassistance.de)
 *
        ...
        play: function (url, options) {
            ...
            if (options.track) {
                if (this.tracks[options.track]) {
                    var current = this.tracks[options.track]; // TODO check when Stop is avaiable, certainly not on a jQuery object
                    current[0].Stop && current[0].Stop();
                    current.remove();
                }
            }
            var element = $.browser.msie ? $('<bgsound/>').attr({
                src: options.url,
                loop: 1,
                autostart: true
